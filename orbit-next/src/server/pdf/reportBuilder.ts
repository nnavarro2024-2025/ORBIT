...
export async function buildAdminReportPDF(input: PDFReportInput): Promise<Uint8Array> {
  ...
  for (const section of input.sections) {
    ...
    if (section.subtitle) {
      const subtitleHeight = drawWrappedText(ctx.page, section.subtitle, PAGE_MARGIN, ctx.cursorY, ctx.page.getWidth() - PAGE_MARGIN * 2, 11, bodyFont, rgb(0.4, 0.4, 0.4));
  ...
  if (input.footer) {
    ...
    drawWrappedText(ctx.page, input.footer, PAGE_MARGIN, ctx.cursorY, ctx.page.getWidth() - PAGE_MARGIN * 2, 10, bodyFont, rgb(0.4, 0.4, 0.4));
  }
...
function drawWrappedText(page: PDFPage, text: string, x: number, y: number, width: number, size: number, font: PDFFont, color = rgb(0, 0, 0)): number {
...
function renderTable(pdfDoc: PDFDocument, ctx: PageContext, rows: string[][], font: PDFFont, headerFont: PDFFont): { ctx: PageContext; height: number } {
  if (rows.length === 0) {
    return { ctx, height: 0 };
  }
